...
| Need | Approach |
|------|----------|
| MCP server needs an API token or password in **env** | Run the **MCP server process** (or Cursor) under `op run` so env values resolve from vault fields; see [reference.md](reference.md). |
...
- **Project MCP:** many setups use [`.cursor/mcp.json`](reference.md) at the repo root (confirm in [Cursor documentation](https://cursor.com/docs) for your version).
...
Copy-paste JSON patterns, env var names, and troubleshooting: [reference.md](reference.md).
